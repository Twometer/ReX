...
fn main() {
    ...
    let samples: Vec<_> = SAMPLES
        .iter()
        .cloned()
        .map(|tex| parse(dbg!(tex)).unwrap())
        .collect();
    let fonts: Vec<_> = fs::read_dir("fonts")
        .unwrap()
        .filter_map(|e| e.ok())
        .filter_map(|entry| {
            fs::read(entry.path())
                ...
                .and_then(|data| {
                    font::parse(&data)
                        .ok()
                        .and_then(|f| f.downcast_box::<OpenTypeFont>().ok())
                })
    ...
    for (row, (font, path)) in fonts.iter().enumerate() {
        ...
        if let Ok(node) =
            engine::layout(&parse(&name).unwrap(), layout_settings).map(|l| l.as_node())
        {
